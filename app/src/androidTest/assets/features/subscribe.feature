Feature: Subscribe feature
  ...
  Scenario Outline: Search for shows from home page, view a show's details, and subscribe to it
    ...
    And The API responds to request "/podcastdetails?feedUrl=http%3A%2F%2Ffeeds.ign.com%2Fignfeeds%2Fpodcasts%2Fgamescoop%2F&maxEpisodes=10" with code 200 and body "game_scoop_show_details.json"
    And The API responds to request "/podcastupdate?feedUrl=http%3A%2F%2Ffeeds.ign.com%2Fignfeeds%2Fpodcasts%2Fgamescoop%2F&publishTimestamp=0" with code 200 and body "game_scoop_show_update.json"
    And The API responds to request "/podcastdetails?feedUrl=http%3A%2F%2Ffeeds.ign.com%2Fignfeeds%2Fpodcasts%2Fvideo%2Fdailyfix%2F&maxEpisodes=10" with code 200 and body "daily_fix_show_details.json"
    And The API responds to request "/podcastupdate?feedUrl=http%3A%2F%2Ffeeds.ign.com%2Fignfeeds%2Fpodcasts%2Fvideo%2Fdailyfix%2F&publishTimestamp=0" with code 200 and body "daily_fix_show_update.json"
    ...
    And I press enter
    And I scroll and click on text "IGN.com - Game Scoop!"
    And I click on "com.vmenon.mpo.search_feature.fab"
    Then I should see text "You have subscribed to this show" on the display
    When I click on content description "Navigate up"
    And I click on text "IGN.com - Daily Fix (Video)"
    And I click on "com.vmenon.mpo.search_feature.fab"
    Then I should see text "You have subscribed to this show" on the display
    Examples:
      | Keyword |
  ...
  Scenario Outline: Check download queued for subscribed show
    ...
    And I should see text "Former EA CEO Calls Devs…Idiots' - IGN Daily Fix" on the display
    When I wait for episode "Former EA CEO Calls Devs…Idiots' - IGN Daily Fix" to finish downloading
    Then I should not see text "Former EA CEO Calls Devs…Idiots' - IGN Daily Fix" on the display
    Examples:
      |
      |
  ...
  Scenario Outline: Should see subscribed shows on Home
    ...
    And I should see content description "IGN.com - Game Scoop!" on the display
    Examples:
      |
      |
